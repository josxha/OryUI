...
# **GetCourierMessage**
> Message GetCourierMessage (string id)
...
{
    // Get a Message
    ApiResponse<Message> response = apiInstance.GetCourierMessageWithHttpInfo(id);
...
# **ListCourierMessages**
> List&lt;Message&gt; ListCourierMessages (long? pageSize = null, string? pageToken = null, CourierMessageStatus? status = null, string? recipient = null)
...
namespace Example
{
    public class ListCourierMessagesExample
    {
        public static void Main()
        {
            ...
            var status = new CourierMessageStatus?(); // CourierMessageStatus? | Status filters out messages based on status. If no value is provided, it doesn't take effect on filter. (optional) 
...
{
    // List Messages
    ApiResponse<List<Message>> response = apiInstance.ListCourierMessagesWithHttpInfo(pageSize, pageToken, status, recipient);
    Debug.Write("Status Code: " + response.StatusCode);
    Debug.Write("Response Headers: " + response.Headers);
    Debug.Write("Response Body: " + response.Data);
}
...
| **status** | [**CourierMessageStatus?**](CourierMessageStatus?.md) | Status filters out messages based on status. If no value is provided, it doesn&#39;t take effect on filter. | [optional]  |
